...
async def get_gender(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    reply_keyboard = [["Algebra", 'Calculus', 'Programming', 'Chemistry', 'History', 'Philosophy', 'Physics', 'Ecomonics']]
...
async def get_subjectstrong(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f'User: {user.first_name}, Strong Subject: {update.message.text}')
...
async def get_subjectweak(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    logger.info(f'User: {user.first_name}, Weak Subject: {update.message.text}')
...
async def get_bio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        db_table_add_user(userdata['user_id'], userdata['user_name'], userdata['user_surname'], userdata['username'], userdata['user_bio'], userdata['subject_strong'], userdata['subject_weak'], userdata['user_gender'])
        logger.info(f"User: {userdata['username']} added to the db succesfully.")
    except Exception as e:
        logger.error(f'Failed to add user: {userdata["username"]} to the db. ERROR: {e}')
...
def db_table_add_user(user_id: int, user_name: str, user_surname: str, username: str, user_bio: str, subject_strong: str, subject_weak: str, user_gender: str):
    cursor.execute('INSERT INTO chatusers (user_id, user_name, user_surname, username, user_bio, subject_strong, subject_weak, user_gender) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', (user_id, user_name, user_surname, username, user_bio, subject_strong, subject_weak, user_gender))
    conn.commit()


def db_table_edit_user(user_id: int, db_key, db_value):
    try:
        cursor.execute(f"Update chatusers set {db_key} = {db_value} where user_id = {user_id}")
    except Exception as e:
